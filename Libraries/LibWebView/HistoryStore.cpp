...
Vector<HistoryEntry> HistoryStore::list_all_entries() const
{
    ...
    auto result = m_persisted_storage.has_value()
        ? m_persisted_storage->list_all_entries()
        : m_transient_storage.list_all_entries();
...
void HistoryStore::remove_entry_by_url(String const& url)
{
    ...
    if (m_persisted_storage.has_value())
        m_persisted_storage->remove_entry_by_url(url);
    else
        m_transient_storage.remove_entry_by_url(url);
}
...
{
    database.execute_statement(statements.delete_entry_by_url, {}, url);
}
